...
/**
 * Module names as they appear in chain configuration JSON files
 * Use these when looking up modules in chain.modules array
 */
export enum ModuleName {
  AUTO_EARN = 'autoEarnModule',
  AUTO_SWAP = 'autoSwapModule',
  AUTO_BRIDGE = 'autoBridgeModule'
}

/**
 * Module types for orchestration and pathfinding
 * Use these for identifying module functionality
 */
export enum ModuleType {
  AUTO_EARN = 'AUTO_EARN', // Aave deposits
  AUTO_SWAP = 'AUTO_SWAP', // Uniswap swaps
  AUTO_BRIDGE = 'AUTO_BRIDGE', // Cross-chain bridging
  VERIFIABLE_AI_AGENT = 'VERIFIABLE_AI_AGENT' // Future AI agent module
}
    ...
    try {
      ...
      for (const file of jsonFiles) {
        try {
          ...
          logger.info(`Loaded module config: ${module.name} from ${file}`, 'ModuleManager');
        } catch (error) {
