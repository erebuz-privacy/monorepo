// CCIP (Cross-Chain Interoperability Protocol) Utilities
...
import { OFFCHAIN_RESOLVER_ABI, RESOLVER_ABI } from '../config/web3/abis';
...
export async function encodeEnsOffchainResponse(
  ...
  const abiItem: AbiItem | undefined = RESOLVER_ABI.find(
    (abi: AbiItem) =>
      abi.type === 'function' &&
      'name' in abi &&
      abi.name === query.functionName &&
      'inputs' in abi &&
      abi.inputs.length === query.args.length
