...
contract TestHelper is CheatCodes, EventsAndErrors {
    ...
    AutoEarn internal AUTO_EARN_MODULE;
    ...
    function setupTestEnvironment() internal virtual {
        ...
        // deployTestContracts(); // Commented out - override in specific tests if needed
    ...
    //     AUTO_EARN_MODULE = new AutoEarn(
    //         vm.addr(2), // authorized relayer
    //         address(0x4200000000000000000000000000000000000006), // WETH address (Base Sepolia)
    //         address(this) // owner
    //     );

    //     // Set up config for AutoEarn module (Base Sepolia testnet)
    ...
    //         sourceTokenAddress: 0x036CbD53842c5426634e7929541eC2318f3dCF7e, // USDC address
    //         vaultAddress: 0x07eA79F68B2B3df564D0A34F8e19D9B1e339814b // Aave V3 Pool address
    //     });
    //     AUTO_EARN_MODULE.setConfig(configs);

    //     // Calculate config hash for AutoEarn module
    //     uint256 configHash = uint256(keccak256(abi.encode(configs)));

    //     // Set the AutoEarn module address and init data
    //     address[] memory moduleAddresses = new address[](1);
    //     bytes[] memory moduleInitData = new bytes[](1);
    //     moduleAddresses[0] = address(AUTO_EARN_MODULE);
    //     moduleInitData[0] = abi.encode(configHash);

    //     // Update the ACCOUNT_IMPLEMENTATION with AutoEarn module
